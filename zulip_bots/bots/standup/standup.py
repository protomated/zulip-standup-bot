            ...
            if user_id in prompt.get('pending_responses', []):
                ...
                if self.use_database:
                    try:
                        ...
                        self._handle_response_in_memory(bot_handler, user_id, user_email, stream_id, standup_date, content, prompt)
                else:
    ...
    def _handle_setup_command(self, message: Dict[str, Any], bot_handler: AbstractBotHandler, args: List[str]) -> None:
        ...
        if self.use_database:
            try:
                ...
                bot_handler.send_reply(message, "There was an error setting up the standup. Please try again later.")
                return
        else:
            # Use in-memory storage
...
"""
    ...
    def _handle_timezone_command(self, message: Dict[str, Any], bot_handler: AbstractBotHandler, args: List[str]) -> None:
        ...
        if not self._is_valid_timezone(timezone_str):
            bot_handler.send_reply(message, f"Invalid timezone: {timezone_str}. Please use a valid timezone like 'America/New_York'.")
            return
...
"""
    ...
    def _handle_config_command(self, message: Dict[str, Any], bot_handler: AbstractBotHandler, args: List[str]) -> None:
        ...
        if option == 'prompt_time':
            ...
            bot_handler.send_reply(message, f"Unknown configuration option: {option}. Valid options are: prompt_time, cutoff_time, reminder_time.")
    ...
    def _generate_and_post_summary(self, stream_id: str) -> None:
        ...
        if self.use_database:
            try:
                ...
                logging.info(f"Retrieved {len(responses)} responses from database for stream {stream_id} on {today}")
            except Exception as e:
